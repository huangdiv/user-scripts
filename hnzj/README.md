...
2. ~~第一次成功加载视频播放器时会弹出脚本运行提示如下图，此时要**点击确定，视频才开始自动静音播放**，之后没有其他任何提示。~~1.9版去掉这个提示了。
...
## 【更新日志】
### 2020年8月2日更新：
- 发布1.9版，新增支持自动进入未完成的课程，学完一个课程自动返回课程列表，学习下一个，直到学完所有已选课程。
- 去掉进入时，脚本的弹窗提示。可按F12进入console界面查看脚本运行提示。
