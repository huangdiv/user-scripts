...
(function() {
    ...
    function playH5(){
        if (document.getElementsByClassName("pv-video")[0].paused){
            document.getElementsByClassName("pv-video")[0].play();
       }
    }
    ...
    function toNextH5(){
        ...
        setTimeout(function(){
            ...
            if(text.match(/[0-9]+%/)[0]=="100%"){
                seekH5(document.getElementsByClassName("pv-video")[0].duration-1);
                setTimeout(playH5,1000);
                console.log('视频播放结束，即将播放下一个视频');
            }else{
                setTimeout(arguments.callee, 5000);
    ...
    function seekH5(sec){
        document.getElementsByClassName("pv-video")[0].currentTime=sec;
    }
    //++++++++++++++++++
    ...
    function start(){
        ...
        if (first==1){
            first=0;
            alert('-----河南专技在线辅助-----\n【自动静音播放|暂停时进度继续增加|自动播放下一个视频|自动答题(去掉答题弹窗)】\n-----作者：huangdiv-----\n-----网址：huangdiv.com-----\n-----版本：1.6-----\n-----2020年4月30日-----\n\n-----点击确定后开始运行------');
        ...
        if (document.getElementsByClassName("pv-video")[0]){
            setTimeout(fixFucH5,1000);//修改功能
            document.getElementsByClassName("pv-video")[0].autoplay=true;//设置加载后自动播放
            setTimeout(playH5,2000);//播放
            document.getElementsByClassName("pv-video")[0].volume=0;//静音
            console.log('切换视频后自动开始播放...');
            setTimeout(hideH5,3000);
            setTimeout(toNextH5,4000);//检测进度
        }
    }
    ...
    window.s2j_onPlayerInitOver=start;//脚本依赖这个播放器初始化功能，每当切换到下一个视频，自动执行以上主要功能区。
    //++++++++++++++++++
