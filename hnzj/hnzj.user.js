...
(function () {
    ...
    function toNextH5() {
        ...
        setTimeout(function () {
            ...
            if (text.match(/[0-9]+%/)[0] == "100%") {
                ...
                if (playerH5.currentTime == playerH5.duration) {
                    for (var i = 0; i < $(
                            'button[id][class="btn btn-primary"][style="display:inline-block"]').length; i++) {
                        if ($('button[id][class="btn btn-primary"][style="display:inline-block"]')[i].innerText ==
                            "返回课程列表") {
                            console.log('课程进度完成，即将返回课程列表');
                            $('button[id][class="btn btn-primary"][style="display:inline-block"]')[i].click();
                            setTimeout(enterCourse, 5000);
                        }
                    }
                }
    ...
    function enterCourse() {
        for (var i = 0; i < document.getElementsByClassName("item-box").length; i++) {
            if (document.getElementsByClassName("sr-only")[i].innerText != "100.0%") {
                document.getElementsByClassName("item-box")[i].click();
                break;
            }
        }
        setTimeout(function () {
            console.log('恭喜您,本课程已完成！');
            alert('恭喜您，本课程已完成！\n-----by:河南专技在线辅助-----');
        }, 5000);
    }
    var player,
