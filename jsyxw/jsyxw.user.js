...
(function () {
    ...
    function insertLogoAfter(CssSelector, text) {
        ...
        if (!document.querySelector("#yxwfz-logo")) {
            document.querySelector(CssSelector).insertAdjacentHTML('afterEnd',
                ...
                "<span id='yxwfz-text' style= 'font-weight: 300;color: #00ff00;'>" + text +
                "</span></span></a>"
            );
        } else {
    ...
    function courseList() {
        ...
        if (minutesTotal > 0) {
            ...
            // 插入辅助标识
            insertLogoAfter("span.sub-title", "目标：" + minutesTotal + "分钟");
            for (var i = 0; i < courses.length; i++) {
                ...
                if (jindubi == 1) {
                    ...
                    document.querySelector("#yxwfz-text").innerHTML = "已打开第" + (i + 1) +
                        "个课程，将在<span id = yxwfz-minutes>" + minutes.toFixed(2) + "</span>分钟后刷新";
                    console.log("已打开列表中第" + (i + 1) + "个看课课程");
                    console.log("计时器已启动：");

                    function countMinute() {
                        document.querySelector("#yxwfz-minutes").innerText = minutes;
                        minutes = minutes - 1;
                        console.log("将在" + (minutes + 1) + "分钟后刷新本列表页");
